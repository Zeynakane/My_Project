...
def Inscription(client:socket, buffer:int):
    liste = list()
    req = "Veuillez saisir votre nom de famille svp : " 
    ...
    while requete1 != requete2:
        ...
        if requete1 != requete2:
            req = "Les mots de passe ne sont pas les mêmes !!"
            client.send(req.encode("utf-8")) #3
        else:
            req = "Les mots de passe sont les mêmes !!"
            client.send(req.encode("utf-8")) #3
...
def openServer(host,
    ...
    while True:    
        ...
        reponse = "Veuillez taper 1 pour vous inscrire ,2 pour vous connecter, ou 3 pour quitter "
